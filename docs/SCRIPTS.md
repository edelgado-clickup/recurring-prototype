...
We primarily use `.sh` scripts instead of npm commands because:

- **✅ Permission Handling**: Scripts avoid npm workspace permission issues
- **✅ Reliability**: Better handling of paths and environments
- **✅ Port Management**: Automatically kills processes on ports before starting
- **✅ Gallery Integration**: Auto-syncs gallery with prototypes
- **✅ Simplicity**: One command does everything

### Recommended Approach:
```bash
✅ Use: ./start-prototype.sh calendar
```

### Fallback Option:
```bash
⚠️ Fallback: npm run dev -w @prototypes/calendar
```

**Note:** If scripts aren't working (permissions, platform issues), npm commands can be used as a fallback. However, you may need to manually handle port conflicts and gallery syncing.
...
**⚠️ Note:** These are fallback options if shell scripts don't work. Shell scripts are recommended for reliability.
...
- Doesn't auto-kill existing processes
- No gallery integration

---

### Start Gallery (Fallback)
```bash
npm run gallery
```
**What it does:**
...
### Start Specific Prototype (Fallback)
```bash
...
- Doesn't check/kill existing processes on port
- Manual gallery sync required
